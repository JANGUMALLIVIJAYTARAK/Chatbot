...
def get_gemini_response(query: str, context_text: str, model_name: str = None, user_gemini_api_key: str = None, **kwargs) -> tuple[str, str | None]:
    if not genai: raise ConnectionError("Gemini SDK not installed.")
    if not user_gemini_api_key: raise ConnectionError("User Gemini API key is required but was not provided.")
    genai.configure(api_key=user_gemini_api_key)
    
    prompt = _SYNTHESIS_PROMPT_TEMPLATE_STR.format(query=query, context=context_text)
    ...
    logger.info(f"Calling Gemini for synthesis (Model: {model.model_name}) using per-request key.")
...
def get_ollama_response(query: str, context_text: str, model_name: str = None, **kwargs) -> tuple[str, str | None]:
    # This function remains unchanged as it doesn't use an API key
    if not ollama_available: raise ConnectionError("Ollama dependencies not available.")
    prompt = _SYNTHESIS_PROMPT_TEMPLATE_STR.format(query=query, context=context_text)
    model = ChatOllama(base_url=OLLAMA_BASE_URL, model=model_name or DEFAULT_OLLAMA_MODEL)
    logger.info(f"Calling Ollama for synthesis (Model: {model.model}).")
    try:
...
def get_groq_llama3_response(query: str, context_text: str, model_name: str = None, user_grok_api_key: str = None, **kwargs) -> tuple[str, str | None]:
    if not Groq: raise ConnectionError("Groq SDK not installed.")
    if not user_grok_api_key: raise ConnectionError("User Groq API key is required but was not provided.")
    # Create a client instance for this specific request
    local_groq_client = Groq(api_key=user_grok_api_key)
    ...
    logger.info(f"Calling Groq for synthesis (Model: {model}) using per-request key.")
...
def generate_response(llm_provider: str, query: str, context_text: str, user_gemini_api_key: str = None, user_grok_api_key: str = None, **kwargs) -> tuple[str, str | None]:
    ...
    return provider_map[llm_provider](**call_args)